...
(d.name.toLowerCase().includes(q) ||
 ...
 d.keywords?.some(k => k.toLowerCase().includes(q)))
  ...
  const ccrColor = () => {
    if (ccr >= 90) return { bg: "#EAF3DE", text: "#27500A", border: "#639922", stage: "G1（正常または高値）" };
    if (ccr >= 60) return { bg: "#E6F1FB", text: "#0C447C", border: "#378ADD", stage: "G2（正常または軽度低下）" };
    if (ccr >= 45) return { bg: "#FAEEDA", text: "#633806", border: "#EF9F27", stage: "G3a（軽度〜中等度低下）" };
  ...
  return (
    <div style={{ fontFamily: "var(--font-sans)", padding: "1rem 0", color: "var(--color-text-primary)" }}>
      ...
      <p style={{ fontSize: 12, color: "var(--color-text-secondary)", margin: "0 0 1.25rem" }}>
        腎機能低下時に最も注意の必要な薬剤投与量一覧（2026年改訂・第39版）に準拠
      </p>
      ...
      <div style={{ background: "var(--color-background-secondary)", borderRadius: 12, padding: "1rem 1.25rem", marginBottom: "1.25rem" }}>
        ...
        <input
          ...
          step="1"
          value={ccr}
          onChange={e => setCcr(Number(e.target.value))}
          style={{ width: "100%", marginBottom: 10 }}
        />
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
          {["0", "15", "30", "45", "60",　"75", "90"].map((l, i) => (
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
		...
		>
		  {drug.category}
                ...
                {rec && (
                  <div style={{ marginTop: 8, background: s.bg, border: `0.5px solid ${s.border}`, borderRadius: 8, padding: "7px 10px" }}>
              ...
              {open && (
                <div style={{ borderTop: "0.5px solid var(--color-border-tertiary)", padding: "0.9rem 1.1rem", fontSize: 12 }}>
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                      <thead>
                        <tr style={{ background: "var(--color-background-secondary)" }}>
                          <th style={{ padding: "4px 8px", textAlign: "left", borderRadius: "4px 0 0 0", fontSize: 11, fontWeight: 500 }}>
                            CCr (mL/min)
                          </th>
                          <th style={{ padding: "4px 8px", textAlign: "left", borderRadius: "0 4px 0 0", fontSize: 11, fontWeight: 500 }}>
                            投与法
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        {[...drug.ranges].sort((a, b) => b.ccr[0] - a.ccr[0]).map((r, i) => {
                          ...
                          return (
                            <tr
                              key={i}
                              style={{ background: isActive ? s.bg : "transparent", borderTop: "0.5px solid var(--color-border-tertiary)" }}
                            >
                              <td
                                style={{
                                  padding: "5px 8px",
                              ...
                              <td style={{ padding: "5px 8px", fontSize: 11, color: isActive ? s.text : "var(--color-text-primary)" }}>
                                {isActive && <span style={{ fontWeight: 600 }}>▶ </span>}
                                {r.dose}
                              </td>
                            </tr>
                          );
                  ...
                  {drug.note && (
                    <div style={{ background: "#FAEEDA", border: "0.5px solid #FAC775", borderRadius: 8, padding: "7px 10px", marginTop: 8 }}>
      ...
      >
        <p style={{ fontSize: 11, color: "var(--color-text-secondary)", margin: 0 }}>
          ⚠ 本ツールは「腎機能低下時に最も注意の必要な薬剤投与量一覧（2026年改訂・第39版）」を参考に作成したものです。実際の投与判断は必ず担当医師・薬剤師が行い、最新の添付文書・ガイドラインをご確認ください。
        </p>
      </div>
    </div>
  );
